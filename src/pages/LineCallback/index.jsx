...
const LineCallback = () => {
  ...
  useEffect(() => {
    ...
    if (error || !code) {
      setMessage(error ? 'LINE 登入失敗' : '缺少授權碼');
      setLoading(false);
      return;
    }
  ...
  const handleExchange = async (code) => {
    try {
      const res = await fetch('https://us-central1-fu-mart-8.cloudfunctions.net/lineLoginCallback', {
        method: 'POST',
      ...
      setAvatar(data.avatar); // <--- 新增這一行，確保圖片網址被保存
      setShowConfirm(true);

    } catch (err) {
      console.error("LINE Exchange Error:", err);
      setMessage('登入失敗');
      setErrorDetail(err.message === "invalid_grant" ? "驗證碼已過期，請重新嘗試" : err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitForm = async () => {
    ...
    try {
      const res = await fetch('https://us-central1-fu-mart-8.cloudfunctions.net/createLineUser', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          lineUserId,
  ...
  return (
    <div className="signup-page"> {/* 使用 signup-page 確保繼承背景與佈局 */}
      <div className="logo-container" onClick={() => navigate('/')}>
        <img src={fumartTextLogo} alt="Fü-Mart" />
      </div>

      <div className="callback-content">
        <div className="signup-card"> {/* 使用 signup-card 確保寬度與陰影一致 */}
          {loading ? (
            ...
            <div className="confirm-box">
              ...
              <p className="signup-subtitle" style={{ color: '#e5341d' }}>這個 LINE 帳號尚未與 Fü-Mart 綁定</p>
            ...
            <div className="signup-step1">
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="dob-section" style={{ marginTop: '16px' }}>
                  {/* <label className="dob-label">生日</label> */}
